jQuery(document).ready(function($) {
	...
	if (!$('#lemur_slider_slides #slides .slide').length) {
	...
	//Select skin
	//$(".lemur-chosen").chosen({disable_search: true});
	
	//Image Scale Mode
	$('.lemur-slider-option.image-scale-option .image-scale-options a').click(function(){
		var val = $(this).data('type');
		$(".lemur-slider-option.image-scale-option select").val(val);
	...
	$('.lemur_slider-auto-update').submit(function(e) {
		e.preventDefault();

		$('.lemur_slider-auto-update tfoot span').text('Validating ...').css('color', '#333');

		$.post( ajaxurl, $(this).serialize(), function(data) {

			data = jQuery.parseJSON(data);

			$('.lemur_slider-auto-update tfoot span').text(data['message']);

			if(data['success'] == true) {
				$('.lemur_slider-auto-update tfoot span').css('color', '#4b982f');
			} else {
				$('.lemur_slider-auto-update tfoot span').css('color', '#c33219');
			}
		});
	});
	
	$('.lemur-slider-unlicenced_copy,.lemur-slider-licenced_copy').click(function(){
		$('.lemur_slider-auto-update').show();
		return false;
	});
	
	//Auto slide type	
	$('#lemur_slider_slides').on('click', '.auto-slide-type a', function(e){
	...
	//Advanced settings
	$('.lemur-slider-advanced-settings').click(function(){
		$('.lemur-slider-advanced-settings-section').toggle();
		return false;
	});
